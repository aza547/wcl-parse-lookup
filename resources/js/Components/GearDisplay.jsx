export const GearDisplay = ({gear}) => {
  ...
  const renderSlotIcon = (item) => {
    ...
    const highlightClassName = item.id === 228411 ? " border-4 animate-pulse duration-2000 border-yellow-500" : "";

    return (
      <div className="slot bg-gray-300 w-10 h-10 border border-gray-400 flex justify-center items-center" key={item.id}>
        <a href={tooltipUrl} className={item.quality}>
          <img src={`https://assets.rpglogs.com/img/warcraft/abilities/${item.icon}`} className={highlightClassName}></img>
        </a>
      </div>
    )
  }
